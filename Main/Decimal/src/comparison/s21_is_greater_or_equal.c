...
int s21_is_greater_or_equal(s21_decimal dec, s21_decimal dec2) {
  int flag = 0;
  ...
  if (s21_is_equal(dec, dec2) != 1) {
    if ((dec.bits[3] == 0) && (dec2.bits[3] == 0)) {
      ...
      if ((flag == -1) && (s21_is_equal(dec, dec2) == 1))
        flag = 1;
      else if (flag == 0)
        flag = 1;
      else
      ...
      flag = s21_NoScaleIsCompar(dec, dec2);
      if ((flag == -1) && (s21_is_equal(dec, dec2) == 1)) flag = 1;
    } else if (scale != 0 || scale_2 != 0)
      flag = s21_IsGreaterOrEqualScale(dec, dec2, is_negat, is_negat_2);
  } else
    flag = 1;
  return flag;
}
